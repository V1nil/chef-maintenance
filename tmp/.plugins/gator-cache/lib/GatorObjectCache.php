...
class GatorObjectCache
{
    ...
    {
        $this->storage->flush();//will flush the entrie cache, every group
    }

    public function get($key, $group = 'default')
    {
        if (empty($group)){ // apparently some callers are sending an empty string
            $group = 'default';
        }
 ...
 */
    ...
    {
		if (false === ($value = $this->get($key, $group))) {
            return false;
        }
    ...
    public function stats() {
		$this->saveData();
        if ((defined('DOING_AJAX') && DOING_AJAX) || (false === ($config = GatorCache::getConfig(ABSPATH . 'gc-config.ini.php'))) || !$config->get('debug')) {
            // no debug
            return;
        }
