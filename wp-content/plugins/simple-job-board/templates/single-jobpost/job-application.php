        ...
        <div class="sjb-row">
            <div class="sjb-col-md-10 sjb-col-sm-12 sjb-col-xs-12">
                ...
                if (NULL != $keys):
                    foreach ($keys as $key):
                        if (substr($key, 0, 7) == 'jobapp_'):
                            ...
                            $required_field_asterisk = isset($val['optional']) ? "checked" === $val['optional'] ? '<span id="sjb-required">*</span>' : "" : '<span id="sjb-required">*</span>';
                            ...
                            switch ( $val['type'] ) {
                                case 'text':
                                    ...
                                    . '<input type="text" name="' . $name . '" class="sjb-form-control ' . $required_class . '" id="' . $id . '" ' . $is_required . ' >'
                                    . '</div>';
                                    break;
                                case 'text_area':
                                    echo '<div class="sjb-form-group">'
                                    . '<label class="sjb-label-control" for="' . $key . '">' . $label . $required_field_asterisk . '</label>'
                                    . '<textarea name="' . $name . '" class="sjb-form-control ' . $required_class . '" id="' . $id . '" ' . $is_required . '></textarea>'
                                    . '</div>';
                                    break;
                                case 'email':
                                    echo '<div class="sjb-form-group">'
                                    . '<label class="sjb-label-control" for="' . $key . '">' . $label . $required_field_asterisk . '</label>'
                                    . '<input type="email" name="' . $name . '" class="sjb-form-control sjb-email-address ' . $required_class . '" id="' . $id . '" ' . $is_required . '><span class="sjb-invalid-email">' . __('A valid email address is required.', 'simple-job-board') . '</span>'
                                    . '</div>';
                                    break;
                                case 'phone':
                                    echo '<div class="sjb-form-group">'
                                    . '<label class="sjb-label-control" for="' . $key . '">' . $label . $required_field_asterisk . '</label>'
                                    . '<input type="tel" name="' . $name . '" class="sjb-form-control sjb-phone-number ' . $required_class . '" id="' . $id . '" ' . $is_required . '><span class="sjb-invalid-phone" id="' . $id . '-invalid-phone">' . __('A valid phone number is required.', 'simple-job-board') . ' </span>'
                                    . '</div>';
                                    break;
                                case 'date':
                                    echo '<div class="sjb-form-group">'
                                    . '<label class="sjb-label-control" for="' . $key . '">' . $label . $required_field_asterisk . '</label>'
                                    . '<input type="date" name="' . $name . '" class="sjb-form-control ' . $required_class . '" id="' . $id . '" ' . $is_required . '>'
                                    . '</div>';
                                    break;
                                case 'radio':
                                    if ($val['options'] != '') {
                                        ...
                                        foreach ($options as $option) {
                                            echo '<label class="sjb-label-control small"><input type="radio" name="' . $name . '" class=" ' . $required_class . '" id="' . $id . '" value="' . $option . '"  ' . job_board_is_checked($i) . ' ' . $is_required . '>' . $option . ' </label> ';
                                            $i++;
                                        }
                                        echo '</div></div>';
                                    }
                                ...
                                case 'checkbox' :
                                    if ($val['options'] != '') {
                                        ...
                                        foreach ($options as $option) {
                                            echo '<label class="sjb-label-control small"><input type="checkbox" name="' . $name . '[]" class="' . $required_class . '" id="' . $id . '" value="' . $option . '"  ' . $i . ' ' . $is_required . '>' . $option . ' </label>';
                                            $i++;
                                        }
                                        echo '</div></div>';
                                    }
                ...
                $sjb_attach_resume = '<div class="sjb-form-group">'
                        . '<label class="sjb-label-control" for="applicant_resume">' . apply_filters('sjb_resume_label', __('Attach Resume', 'simple-job-board')) . '<span id="sjb-required">*</span></label>'
                        . '<input type="file" name="applicant_resume" id="applicant-resume" class="sjb-attachment "' . apply_filters('sjb_resume_required','required="required"') . '>'
                        . ' <span class="sjb-invalid-attachment" id="file-error-message"></span>'
                        . '</div>';
                ...
                <div class="sjb-form-group" id="sjb-form-padding-button">
                    <button id="jobpost-submit-button"><?php _e('Submit', 'simple-job-board'); ?></button>
                </div>
            </div>    
        </div>
    </form>
