<div class="wrap">
    ...
    <select id="limit-recipe">
        <option value="0"><?php _e( 'Select a recipe', 'wp-ultimate-recipe' ); ?></option>
        <?php
        foreach( $recipes as $recipe ) {
            $selected = $recipe['value'] == $limit_by_recipe_id ? ' selected' : '';
            echo '<option value="' . $recipe['value'] . '"' . $selected . '>' . $recipe['label'] . '</option>';
        }
        ?>
    </select>
    ...
    <?php } else {
        echo '<p>' . __( 'Select a recipe to calculate its nutritional data.', 'wp-ultimate-recipe' ) . '</p>';
    }?>
    <br/>
