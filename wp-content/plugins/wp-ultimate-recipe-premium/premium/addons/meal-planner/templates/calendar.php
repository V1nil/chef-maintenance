...
<table class="wpurp-meal-plan-calendar" data-admin="<?php echo $meal_plan_admin ? 'true' : 'false'; ?>" data-meal-plan-id="<?php echo $meal_plan_id; ?>" data-start-date="<?php echo $start_date->format( 'Ymd' ); ?>" data-end-date="<?php echo $end_date ? $end_date->format( 'Ymd' ) : ''; ?>" data-nbr-days="<?php echo $nbr_of_days; ?>">
        ...
        foreach( $dates as $date => $recipes ) {
            ...
            if( isset( $recipes[$course] ) ) {
                foreach( $recipes[$course] as $recipe ) {
                    ...
                    $nutritional = ' data-calories="' . $recipe_obj->nutritional( 'calories' ). '"';
                    $nutritional .= ' data-fat="' . $recipe_obj->nutritional( 'fat' ). '"';
                    $nutritional .= ' data-carbohydrate="' . $recipe_obj->nutritional( 'carbohydrate' ). '"';
                    $nutritional .= ' data-protein="' . $recipe_obj->nutritional( 'protein' ). '"';

                    echo '<div class="wpurp-meal-plan-recipe" data-recipe="' . $recipe['id'] . '" data-servings="' . $recipe['servings'] . '"' . $nutritional . '>';
    ...
    <tr class="wpurp-meal-plan-header">
        <td colspan="<?php echo $nbr_of_days; ?>"><?php _e( 'Per Person Nutrition Facts', 'wp-ultimate-recipe' ); ?></td>
    </tr>
    <?php if( WPUltimateRecipe::option( 'nutritional_information_unit', 'calories' ) == 'kilojoules') { ?>
    <tr class="wpurp-meal-plan-nutrition-facts wpurp-meal-plan-nutrition-facts-energy">
        <td>
            <span class="wpurp-meal-plan-nutrition-type"><?php _e( 'Energy', 'wp-ultimate-recipe' ); ?></span>
            <span class="wpurp-meal-plan-nutrition-unit">(kJ)</span>
    ...
    <?php } else { ?>
    <tr class="wpurp-meal-plan-nutrition-facts wpurp-meal-plan-nutrition-facts-calories">
        <td>
            <span class="wpurp-meal-plan-nutrition-type"><?php _e( 'Calories', 'wp-ultimate-recipe' ); ?></span>
            <span class="wpurp-meal-plan-nutrition-unit">(kcal)</span>
            <span class="wpurp-meal-plan-nutrition-value"></span>
        </td>
        <?php
        for( $i = 1; $i < $nbr_of_days; $i++ ) {
    ...
    <?php } ?>
    <tr class="wpurp-meal-plan-nutrition-facts wpurp-meal-plan-nutrition-facts-fat">
        <td>
            <span class="wpurp-meal-plan-nutrition-type"><?php _e( 'Fat', 'wp-ultimate-recipe' ); ?></span>
            <span class="wpurp-meal-plan-nutrition-unit">(g)</span>
            <span class="wpurp-meal-plan-nutrition-value"></span>
        </td>
        <?php
        for( $i = 1; $i < $nbr_of_days; $i++ ) {
    ...
    <tr class="wpurp-meal-plan-nutrition-facts wpurp-meal-plan-nutrition-facts-carbohydrate">
        <td>
            <span class="wpurp-meal-plan-nutrition-type"><?php _e( 'Carbohydrates', 'wp-ultimate-recipe' ); ?></span>
            <span class="wpurp-meal-plan-nutrition-unit">(g)</span>
            <span class="wpurp-meal-plan-nutrition-value"></span>
        </td>
        <?php
    ...
    <tr class="wpurp-meal-plan-nutrition-facts wpurp-meal-plan-nutrition-facts-protein">
        <td>
            <span class="wpurp-meal-plan-nutrition-type"><?php _e( 'Protein', 'wp-ultimate-recipe' ); ?></span>
            <span class="wpurp-meal-plan-nutrition-unit">(g)</span>
            <span class="wpurp-meal-plan-nutrition-value"></span>
        </td>
        <?php
