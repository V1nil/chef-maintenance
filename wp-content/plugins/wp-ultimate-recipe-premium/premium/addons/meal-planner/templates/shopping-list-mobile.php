...
<head>
    ...
    <script>
        var ajax_url = '<?php echo WPUltimateRecipe::get()->helper('ajax')->url(); ?>';
        var ajax_nonce = '<?php echo wp_create_nonce( 'wpurp_shopping_list_mobile' ); ?>';
    </script>
</head>
...
<div id="wpurp-meal-plan-shopping-list-mobile-tip">
    ...
    if( WPUltimateRecipe::option( 'meal_plan_shopping_list_email', '0' ) == '1' ) {
        echo '<br/><br/>';
        echo __( 'Or send this link via email for easy access:', 'wp-ultimate-recipe' ) . '<br/>';
        echo '<input id="email" type="text" value="' . $email . '"/>';
        echo '<input id="submit" type="submit" value="' . __( 'Send this Shopping List', 'wp-ultimate-recipe' ) . '"/>';
    }
    ?>
</div>
