...
RecipeMealPlanner.addRecipe = function(recipe) {
    if(recipe.id !== '')
    {
        ...
        var recipe_data = {
            ...
            calories: recipe.element[0].dataset.calories,
            fat: recipe.element[0].dataset.fat,
            carbohydrate: recipe.element[0].dataset.carbohydrate,
            protein: recipe.element[0].dataset.protein,
        ...
        container.slideUp(200, function() {
            ...
            var recipe_block = '<div class="wpurp-meal-plan-recipe" data-recipe="' + recipe_data.id +'" data-servings="' + recipe_data.servings_original + '"  data-calories="' + recipe_data.calories + '" data-fat="' + recipe_data.fat + '" data-carbohydrate="' + recipe_data.carbohydrate + '" data-protein="' + recipe_data.protein + '"><img src="' + recipe_data.image + '"><span class="wpurp-meal-plan-recipe-title">' + recipe_data.name + '</span> <span class="wpurp-meal-plan-recipe-servings">(' + recipe_data.servings_original + ')</span></div>';
...
RecipeMealPlanner.updateNutritional = function(calendar_container) {
    ...
    var calories_per_day = [];
    var energy_per_day = [];
    var fat_per_day = [];
    var carbohydrate_per_day = [];
    var protein_per_day = [];
    var missing = [];
    ...
    if(nutrition_facts.length > 0) {
        calendar_container.find('.wpurp-meal-plan-course').each(function(index_course) {
            jQuery(this).find('.wpurp-meal-plan-recipe-list').each(function(index_day) {
                ...
                if(calories_per_day[index_day] == undefined) calories_per_day[index_day] = 0;
                if(fat_per_day[index_day] == undefined) fat_per_day[index_day] = 0;
                if(carbohydrate_per_day[index_day] == undefined) carbohydrate_per_day[index_day] = 0;
                if(protein_per_day[index_day] == undefined) protein_per_day[index_day] = 0;
                if(missing[index_day] == undefined) missing[index_day] = 0;

                jQuery(this).find('.wpurp-meal-plan-recipe').each(function(index_recipe) {
                    var recipe = jQuery(this);

                    var calories = recipe.data('calories') || 0;
                    var fat = recipe.data('fat') || 0;
                    var carbohydrate = recipe.data('carbohydrate') || 0;
                    var protein = recipe.data('protein') || 0;

                    calories_per_day[index_day] += calories;
                    fat_per_day[index_day] += fat;
                    carbohydrate_per_day[index_day] += carbohydrate;
                    protein_per_day[index_day] += protein;

                    if(calories == 0 && fat == 0 && carbohydrate == 0 && protein == 0) {
                        missing[index_day] += 1;
                    }
                });
            });
        ...
        // Calculate kJ
        for(var i = 0, l = calories_per_day.length; i < l; i++) {
            energy_per_day[i] = parseInt( 4.1868 * calories_per_day[i] );
        }

        RecipeMealPlanner.updateNutritionalValues(nutrition_facts.find('.wpurp-meal-plan-nutrition-facts-calories'), calories_per_day);
        RecipeMealPlanner.updateNutritionalValues(nutrition_facts.find('.wpurp-meal-plan-nutrition-facts-energy'), energy_per_day);
        RecipeMealPlanner.updateNutritionalValues(nutrition_facts.find('.wpurp-meal-plan-nutrition-facts-fat'), fat_per_day);
        RecipeMealPlanner.updateNutritionalValues(nutrition_facts.find('.wpurp-meal-plan-nutrition-facts-carbohydrate'), carbohydrate_per_day);
        RecipeMealPlanner.updateNutritionalValues(nutrition_facts.find('.wpurp-meal-plan-nutrition-facts-protein'), protein_per_day);
