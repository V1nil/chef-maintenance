        ...
        foreach( $recipe_groups as $group )
        {
            ...
            foreach( $group['recipes'] as $recipe )
            {
                if( isset( $recipe_cache[$recipe->ID()] ) ) {
                    $servings = $recipe_cache[$recipe->ID()]['servings_normalized'];
                    $nutritional_calories = isset( $recipe_cache[$recipe->ID()]['nutritional']['calories'] ) ? $recipe_cache[$recipe->ID()]['nutritional']['calories'] : '';
                    $nutritional_fat = isset( $recipe_cache[$recipe->ID()]['nutritional']['fat'] ) ? $recipe_cache[$recipe->ID()]['nutritional']['fat'] : '';
                    $nutritional_carbohydrate = isset( $recipe_cache[$recipe->ID()]['nutritional']['carbohydrate'] ) ? $recipe_cache[$recipe->ID()]['nutritional']['carbohydrate'] : '';
                    $nutritional_protein = isset( $recipe_cache[$recipe->ID()]['nutritional']['protein'] ) ? $recipe_cache[$recipe->ID()]['nutritional']['protein'] : '';
                    $link = $recipe_cache[$recipe->ID()]['link'];
                    $thumbnail = $recipe_cache[$recipe->ID()]['thumbnail'];
                    $title = $recipe_cache[$recipe->ID()]['title'];
                } else {
                    $servings = $recipe->servings_normalized();
                    $nutritional_calories = $recipe->nutritional( 'calories' );
                    $nutritional_fat = $recipe->nutritional( 'fat' );
                    $nutritional_carbohydrate = $recipe->nutritional( 'carbohydrate' );
                    $nutritional_protein = $recipe->nutritional( 'protein' );
                    $link = $recipe->link();
                ...
                $nutritional = ' data-calories="' . $nutritional_calories . '"';
                $nutritional .= ' data-fat="' . $nutritional_fat . '"';
                $nutritional .= ' data-carbohydrate="' . $nutritional_carbohydrate . '"';
                $nutritional .= ' data-protein="' . $nutritional_protein . '"';

                $out .= '<option value="' . $recipe->ID() . '" data-servings="' . $servings . '" data-link="' . $link . '" data-thumb="' . $thumbnail . '"' . $nutritional . '>' . $title . '</option>';
            }
