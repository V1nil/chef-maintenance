...
class WPURP_Custom_Fields extends WPURP_Premium_Addon {
    ...
    public function add_custom_field() {
        ...
        if( strlen($name) > 1 && strlen($key) > 1 ) {
            ...
            do_action( 'wpml_register_single_string', 'wpurp-custom-fields', $key . ' label', $name );
        ...
        if( !$this->fields ) {
            ...
            if( !is_admin() && function_exists( 'icl_object_id' ) ) {
                ...
                foreach( $custom_fields as $key => $values ) {
                    $values['name'] = apply_filters( 'wpml_translate_single_string', $values['name'], 'wpurp-custom-fields', $key . ' label' );
                    $translated_custom_fields[$key] = $values;
                }

                $custom_fields = $translated_custom_fields;
            }
