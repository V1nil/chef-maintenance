...
class WPURP_Parser {
    ...
    protected function parseBlockRecipeTitle( $block )
    {
        $recipe_title = new WPURP_Template_Recipe_Title();
        if( isset( $block->tag ) ) {
            $recipe_title->tag($block->tag);
        }
    ...
    protected function parseBlockRecipeLink( $block )               {
        $link = new WPURP_Template_Recipe_Link();

        $link->link( $block->recipeLink )->title( $block->recipeLinkTitle );

        if( $block->recipeLinkNewPage ) {
            $link->target('_blank');
        }
