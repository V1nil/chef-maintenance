...
class WPUPG_Filter_Dropdown extends WPUPG_Premium_Addon {
            ...
            foreach( $taxonomies as $taxonomy_key ) {
                if( isset( $cache['taxonomies'][$taxonomy_key] ) ) {
                    ...
                    if( $taxonomy ) {
                        ...
                        $placeholder = isset( $dropdown_labels[$taxonomy_key] ) ? $dropdown_labels[$taxonomy_key] : $taxonomy->labels->name;
                        ...
                        $select_options = $this->generate_hierarchical_select( $grid, $category_args, $cache, $taxonomy_key, $options );
    ...
    private function generate_hierarchical_select( $grid, $args, $cache, $taxonomy_key, $options, $level = 0 ) {
        $select = '';
            ...
            if( is_object( $option ) ) {
                ...
                if( array_key_exists( $slug, $cache['taxonomies'][$taxonomy_key] ) ) {
                    ...
                    if( $grid->filter_count() ) {
                        $name = $option->name . ' (' . count( $cache['taxonomies'][$taxonomy_key][$slug] ) . ')';
                    } else {
                        $name = $option->name;
                    }

                    $select .= '<option value="'.$slug.'">'.$indent.$name.'</option>';
                }

                $args['parent'] = $option->term_id;
                $children = get_categories( $args );
                $select .= $this->generate_hierarchical_select( $grid, $args, $cache, $taxonomy_key, $children, $level+1);
            }
        }
